...
function InstallDHCP
{
    ...
    Invoke-Command -Session $Global:SESSION -ScriptBlock {
        ...
        try 
        {
            ...
            if($status.InstallState -eq "Available")
            {
                write-host -fore green "INSTALLING DHCP SERVER FOR [$($Global:REMOTEPCNAME)]" 
            ...
            {
                write-host -fore red "CAN'T INSTALL DHCP ROLE, STATE IS $($status.InstallState)"
            }
        }
        catch
        {
...
$host.ui.RawUI.WindowTitle = "INFRABUILDER Active Directory 2012R2 [V1.0]"
initVariables
startProcedure
