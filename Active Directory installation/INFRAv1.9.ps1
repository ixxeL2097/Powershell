...
function startProcedure
{
    ...
    Write-Host -fore Cyan "INSTALL CERTIFICATION AUTHORITY ? [y/n]"
    $Global:setADCS = Read-Host
	if($Global:setADCS -eq "y")
	{
		askADCS
	}
    else { Write-Host -fore Red "ADCS INSTALLATION SKIPPED" }
...
function tryToInstallReverseZone
{
    do
    { 
...
function joinDomain($DomainName)
{
    Invoke-Command -Session $Global:SESSION -ScriptBlock {
        param($DomainName)
        try
        {
            Write-Host "ADDING $($Global:REMOTEPCNAME) TO $DomainName" -ForegroundColor Green
            Add-Computer -DomainName $DomainName -Credential $Global:ROOTCredentials
        }
        Catch
        {
            $Error.Exception.Message; Break;
        } 
...
function InstallDHCP
{
    ...
    Invoke-Command -Session $Global:SESSION -ScriptBlock {
        param($Global:NETID, $Global:REMOTEPCNAME, $Global:IP, $Global:ROOTCredentials)
        try 
        {
    ...
    } -ArgumentList $Global:NETID, $Global:REMOTEPCNAME, $Global:IP, $Global:ROOTCredentials
...
function InstallDHCPScope
{
    ...
    Invoke-Command -Session $Global:SESSION -ScriptBlock {
        ...
        (
            ...
            #Set-DhcpServerv4Binding -BindingState $True -InterfaceAlias $Global:ADAPTER.name
            Write-Host -fore Red "INCLUSION name : $($Global:SCOPENAME) start : $($Global:STARTRANGE) stop : $($Global:STOPRANGE) subnet mask : $($Global:MASK)"
            Add-DhcpServerv4Scope -Name $Global:SCOPENAME -StartRange $Global:STARTRANGE -EndRange $Global:STOPRANGE -SubnetMask $Global:MASK -ea SilentlyContinue
            Write-Host -fore Red "EXCLUSION name : $($Global:SCOPEID) start : $($Global:STARTEXCLUSION) stop : $($Global:STOPEXCLUSION) subnet mask : $($Global:MASK)"
    ...
    } -ArgumentList $Global:REMOTEPCNAME, $Global:IP, $Global:ADAPTER, $Global:SCOPENAME, $Global:STARTRANGE, $Global:STOPRANGE, $Global:MASK, $Global:DOMAIN, $Global:DNS1, $Global:DNS2, $Global:GATEWAY, $Global:SCOPEID, $Global:STARTEXCLUSION, $Global:STOPEXCLUSION
    Write-Host -fore Green "SCOPE $($Global:SCOPENAME) INSTALLED WITH SUCCESS ON NETWORK $($Global:SCOPEID)/$($Global:MASK)"
}

function InstallCertificationServer
{
    Invoke-Command -Session $Global:SESSION -ScriptBlock {
        param($Global:ADCS_CN)
        ...
        Install-AdcsCertificationAuthority `
            ...
            -CACommonName $Global:ADCS_CN `
            -Confirm:$false `
            -Verbose
    } -ArgumentList $Global:ADCS_CN
    Write-Host -fore Green "CERTIFICATION AUTHORITY INSTALLATION SUCCESS CN : $($Global:ADCS_CN) !!!"
} # ErrorId 0 confirme le succes de l'installation.

function InstallGPO($GPOname)
{
