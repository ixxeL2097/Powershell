...
function Exec
{
	...
	if($Global:setFOREST -eq "y")
	{
        ...
        switch($Global:DCINSTALLMODE)
        {
            ...
            default { Write-Host -fore Red "no information received!" }
        }
        closeSession $Global:SESSION
...
function InstallDomainController
{
    ...
    Invoke-Command -Session $Global:SESSION -ScriptBlock {
        ...
        try 
        {
            ...
            if($status.InstallState -eq "Available")
            {
                write-host -fore green "STARTING SECOND DOMAIN CONTROLLER INSTALLATION [$($Global:DOMAIN)] on serveur [$($Global:REMOTEPCNAME)]..."
                Install-windowsfeature -name AD-Domain-Services -IncludeManagementTools -Verbose
                Import-Module ADDSDeployment
...
Pause
Stop-Transcript -Path "C:\log\Plog.txt"
