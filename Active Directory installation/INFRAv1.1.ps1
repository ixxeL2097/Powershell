...
$NETBIOS            = $null
$KEY                = $null         #password de l'installation du DC
...
function initVariables
{
    ...
    $Global:REMOTEPCNAME        = "SRV-DC1"                                                                                               #hostname du PC distant pour la connexion
    $Global:CIDR                = "24"
    $Global:KEY                 = ConvertTo-SecureString -String "Root365it" -AsPlainText -Force                                          #password de l'installation du DC
...
{
    Invoke-Command -Session $Global:SESSION -ScriptBlock {
        param($Global:DOMAIN, $Global:NETBIOS, $Global:KEY, $Global:INSTALLDNS)
        try 
        {
    ...
    } -ArgumentList $Global:DOMAIN, $Global:NETBIOS, $Global:KEY, $Global:INSTALLDNS
}
