...
{
	Write-Host -fore Red "Do you want to restart computer now (Recommanded) ? [y/n]" 
    $answer = Read-Host 
    if($answer -eq "y")
    {
        $hostname = hostname
...
function SetDNSConfiguration
{
    ...
    if($answer -eq "y")
    {
		...
		foreach ($zones in $zone.ZoneName)
        {  
			if($zones -eq $Global:DOMAIN)
			{
        ...
        if($answer -eq "y")
        {
			$reversed = Get-DnsServerZone -name $Global:DOMAIN
			if(!$reversed.isReverselookupZone)
			{
				catchNetworkID    
				Add-DnsServerPrimaryZone `
			...
			{
				Write-Host -fore Red "Reverse lookup zone already exists for this domain : $($Global:DOMAIN)"
			}
        }
    }      
    else
